  ...
  class PixAttach {
    ...
    getInfo() {
      return {
        ...
        color3: "#4037A6",
        blocks: [
          { opcode: "connect", blockType: Scratch.BlockType.COMMAND, text: "connect to PixAttach [URL] project [PROJECT] token [TOKEN]", arguments: {
            URL: { type: Scratch.ArgumentType.STRING, defaultValue: "ws://localhost:8000/ws" },
            PROJECT: { type: Scratch.ArgumentType.STRING, defaultValue: "my-project" },
            TOKEN: { type: Scratch.ArgumentType.STRING, defaultValue: "development-token" }
          }},
          { opcode: "disconnect", blockType: Scratch.BlockType.COMMAND, text: "disconnect from PixAttach" },
          { opcode: "isConnected", blockType: Scratch.BlockType.BOOLEAN, text: "connected to PixAttach?" },
          "---",
          { opcode: "exposeVariable", blockType: Scratch.BlockType.COMMAND, text: "expose variable [NAME]", arguments: { NAME: { type: Scratch.ArgumentType.STRING, defaultValue: "Score" }}},
          { opcode: "hideVariable", blockType: Scratch.BlockType.COMMAND, text: "hide variable [NAME]", arguments: { NAME: { type: Scratch.ArgumentType.STRING, defaultValue: "Score" }}},
          { opcode: "setVariable", blockType: Scratch.BlockType.COMMAND, text: "set variable [NAME] to [VALUE]", arguments: {
            NAME: { type: Scratch.ArgumentType.STRING, defaultValue: "Score" }, VALUE: { type: Scratch.ArgumentType.STRING, defaultValue: "100" }
          }},
          ...
          { opcode: "connectionError", blockType: Scratch.BlockType.REPORTER, text: "connection error", allowDropAnywhere: true },
          "---",
          { opcode: "whenConnected", blockType: Scratch.BlockType.HAT, text: "when PixAttach connects", isEdgeActivated: false },
          { opcode: "whenDisconnected", blockType: Scratch.BlockType.HAT, text: "when PixAttach disconnects", isEdgeActivated: false },
    ...
    connect(args) {
      ...
      this.socket.addEventListener("open", () => {
        this.connected = true;
        this.sendPacket({ type: "connect", client_type: "project", project_id: this.projectID, token: this.token, exposed_variables: Array.from(this.exposedVariables) });
      ...
      this.socket.addEventListener("close", () => {
        ...
        this.connected = false;
        this.socket = null;
        if (wasConnected) runtime.startHats("pixattach_whenDisconnected");
      });
      this.socket.addEventListener("error", () => this.handleError("WebSocket connection error"));
    }
    ...
    isConnected() { return Boolean(this.connected && this.socket && this.socket.readyState === WebSocket.OPEN); }

    findGlobalVariable(name) {
      const stage = runtime.getTargetForStage();
      if (!stage || !stage.variables) return null;
      const wanted = Scratch.Cast.toString(name);
      return Object.values(stage.variables).find(v => v.name === wanted && v.type === "") || null;
    }

    exposeVariable(args) {
      const name = Scratch.Cast.toString(args.NAME);
      if (!this.findGlobalVariable(name)) { this.handleError(`Global variable "${name}" was not found`); return; }
      this.exposedVariables.add(name);
      if (this.isConnected()) this.sendPacket({ type: "expose_variable", name });
    }

    hideVariable(args) {
      const name = Scratch.Cast.toString(args.NAME);
      this.exposedVariables.delete(name);
      if (this.isConnected()) this.sendPacket({ type: "hide_variable", name });
    }

    isVariableExposed(args) { return this.exposedVariables.has(Scratch.Cast.toString(args.NAME)); }

    setVariable(args) {
      const variable = this.findGlobalVariable(args.NAME);
      if (!variable) { this.handleError(`Global variable "${Scratch.Cast.toString(args.NAME)}" was not found`); return; }
    ...
    sendValue(args) {
      if (!this.isConnected()) { this.handleError("PixAttach is not connected"); return; }
      this.sendPacket({ type: "value", value: args.VALUE });
    }

    receivedValue() { return this.lastValue; }
    receivedCommand() { return this.lastCommand; }
    connectionError() { return this.lastError; }
    ...
    setVariableFromServer(name, value) {
      name = Scratch.Cast.toString(name);
      if (!this.exposedVariables.has(name)) return false;
    ...
    handleMessage(raw) {
      ...
      if (packet.type === "value") {
        ...
        const success = this.setVariableFromServer(packet.name, packet.value);
        this.sendPacket({ type: "set_variable_result", request_id: packet.request_id || "", name: packet.name, success });
